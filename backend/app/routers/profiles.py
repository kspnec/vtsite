...
from app.core.deps import get_approved_user, get_current_user
...
def list_profiles(
    village_area: Optional[str] = Query(None),
...
def get_profile(user_id: int, db: Session = Depends(get_db)):
    ...
    user = db.query(User).filter(User.id == user_id, User.is_approved == True, User.is_active == True).first()
    if not user:

...
def get_leaderboard(
    category: str | None = Query(
        None,
        description="school_primary|school_middle|school_high|school_higher|college|working|all",
    ),
    ...
    if category == "school_primary":
        q = q.filter(
            User.education_stage == EducationStage.school,
            User.school_grade >= 1,
            User.school_grade <= 5,
        )
        ...
        q = q.filter(
            User.education_stage == EducationStage.school,
            User.school_grade >= 9,
            User.school_grade <= 10,
        )
    elif category == "school_higher":
        q = q.filter(
            User.education_stage == EducationStage.school,
            User.school_grade >= 11,
            User.school_grade <= 12,
        )
